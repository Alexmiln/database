...
-- 2. Выдать информацию о клиентах гостиницы “Космос”, проживающих в номерах категории “Люкс” на сегодня
SELECT c.id_client, c.name, c.phone, rib.checkin_date AS checkin_date, rib.checkout_date AS checkout_date
FROM client c
...
WHERE h.name = 'Космос' AND rc.name = 'Люкс' AND CURDATE() - INTERVAL 3 YEAR BETWEEN rib.checkin_date AND rib.checkout_date - INTERVAL 1 DAY;
-- '- INTERVAL 3 YEAR' исключительно для проверки работоспособности запроса

-- 3. Дать список свободных номеров всех гостиниц на 22 апреля
SELECT r.number AS room_number, h.name AS hotel_name, rc.name AS room_category, r.price AS price FROM room r
...
GROUP BY room_number, hotel_name, room_category
ORDER BY hotel_name;
